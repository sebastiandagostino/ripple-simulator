...
Message::Message(int fromNodeId, int toNodeId, const std::unordered_map<int, NodeState>& data) :
...
const std::unordered_map<int, NodeState>& Message::getData() const {
    return data;
}
...
void Message::addPositions(const std::unordered_map<int, NodeState>& update) {
    ...
    for (auto const& updatePos : update) {
        if (updatePos.first != this->toNodeId) {
            ...
            std::unordered_map<int, NodeState>::iterator msgIt = data.find(updatePos.first);
...
void Message::subPositions(const std::unordered_map<int, NodeState>& received) {
    // we received this information from this node, so no need to send it
    std::unordered_map<int, NodeState>::const_iterator rcvIt;
    for (rcvIt = received.begin(); rcvIt != received.end(); rcvIt++) {
        if (rcvIt->first != this->toNodeId) {
            std::unordered_map<int, NodeState>::iterator msgIt = this->data.find(rcvIt->first);
            if ((msgIt != this->data.end()) && (rcvIt->second.getTimeStamp() >= msgIt->second.getTimeStamp())) {
